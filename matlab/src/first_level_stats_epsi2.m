function first_level_stats_epsi2(inp)

% Modeling:
%   Trial from cue to feedback (fixed duration 0 "event")
%   Feedback (fixed duration 0 "event")
%   Orthogonalized parametric modulators epsi2 epsi3 psi2 psi3 mu3 switch
...
for r = 1:4
	...
	matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(c).pmod(6).name = 'switch';
	matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(c).pmod(6).param = double(strcmp(thist.Switch(ind),'Switch'));
	matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(c).pmod(6).poly = 1;
...
%% Save batch and run
save(fullfile(inp.out_dir,'spmbatch_first_level_stats_epsi2.mat'),'matlabbatch')
